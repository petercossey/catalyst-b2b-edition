...
type GiftCertificateSettings = ResultOf<typeof GiftCertificateSettingsFragment>;

interface GiftCertificatePurchaseTranslator {
  (
    key:
      | 'Form.Errors.amountRequired'
      | 'Form.Errors.amountInvalid'
      | 'Form.Errors.unexpectedSettingsError',
  ): string;
  (
    key: 'Form.Errors.amountOutOfRange',
    values: {
      minAmount: string;
      maxAmount: string;
    },
...
const schema = (
  giftCertificateSettings: GiftCertificateSettings | undefined,
  t: GiftCertificatePurchaseTranslator,
) => {
  return z
    .object({
      senderName: z.string(),
...
export async function addGiftCertificateToCart<F extends Field>(
  ...
  const submission = parseWithZod(formData, {
    schema: schema(
      settingsResp.data.site.settings?.giftCertificates ?? undefined,
      t as unknown as GiftCertificatePurchaseTranslator,
    ),
  });
